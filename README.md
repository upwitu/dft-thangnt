...
every assistant turn, and ignores `enable_thinking` — so the block is always
supplied by the template, never generated by the model. Training on it would
teach the model to emit tokens it is never asked to produce.

`--no-think` (**on by default**) therefore keeps the block in `input_ids`, so the
training sequence matches what the model sees at inference, but masks it out of
`labels`. On a system+user+assistant row against Qwen3 that takes the supervised
span from 15 tokens to 11 — exactly the four `<think>` tokens. Pass `--think` to
supervise the block, e.g. when your data carries real reasoning traces you want
the model to reproduce. On models without `<think>` tokens the flag does nothing.

This is the one place this repo differs in intent from `kd-baselines`, which
reaches the same end through LlamaFactory's `qwen3_nothink` / `qwen3_5_nothink`
templates.
...
--no-think / --think        Mask (default) or supervise <think> blocks
